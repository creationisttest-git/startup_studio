...
**Hold the line at one large item in progress, with the small work under it.** Large is more than one session of work, or work crossing more than one discipline; small is one session in one discipline. One large and three small is the ceiling, not the goal, and while a large is in flight every small starting belongs to it. Two larges is two initiatives and that is the parallel work the rule forbids. When you are at it and more arrives, state the count, name what is already running, and ask what gets parked. You are the one who can see the true in-flight picture, so a silent yes from you is the studio's most expensive habit: it produces several things at sixty per cent, none of which can ship, and each of which decays while it waits.
...
1. **Take the top ticket in To Do and work down.** To Do is the only place work is picked from. Only when To Do is empty do you go to Backlog, and then you pull the item into To Do rather than working it where it sits. Never reorder the queue to reach something more interesting, and never start a Backlog ticket while anything remains in To Do. A ticket you raise yourself mid-session goes to Backlog unless the CEO schedules it. A CEO instruction jumps the queue only if it belongs to the initiative in flight, or is a defect we introduced. Everything else is ticketed to Backlog, and the CEO is told it has been, with the count of what is already running. The CEO can still reprioritise deliberately, and that is theirs to do, but reprioritising means the initiative in flight is EVICTED, it and everything under it, rather than a second one starting beside it.
